...
plot_all <- function(year = 2023, refresh = FALSE,write = FALSE){
    # width and height
    w1 = 8
    h1 = 6
    ...
    path = file.path("~","git","EJData","reports",paste0(year,"-Report"),"images")
    ...
    tab = li[, .(iterations_paper = max(round)), by = ms][, .N,  by = iterations_paper]
    out$iter_paper_bar = ggplot(tab, aes(x = iterations_paper, y = N)) +
        ggplot2::geom_bar(stat="identity", fill = "black") + theme_bw()
    ...
    li[ , sround := round]
    li[sround > 5, sround := 5]
    tab = li[, .(iterations_paper = max(round)), by = ms][, prop.table(table(iterations_paper))]
...
plot_trends <- function(x){
    ...
    l = list()
    l$p1 = ggplot(melt(n, id.vars = "quarter",value.name = "Count"), aes(x = quarter , y = Count )) +
        geom_point() + geom_line() +
        facet_wrap(~variable) + theme_bw()

    n[ , Iterations_Paper := Iterations / Papers]
    l$p2 = ggplot(n, aes(x = quarter , y = Iterations_Paper )) +
...
plot_hours_iters_paper <-function(){
    ...
    l$hours_i = ggplot(x$iterations[hours_spent < 6], aes(x = arrival_date, y = hours_spent)) + geom_point(size = 0.5) +
        geom_line(data = ii, aes(x = arrival_date,y = hours_smooth), size = 1, color = "red") + theme_bw() +
        ...
        geom_vline(xintercept = as.Date("2023-07-01")) + ggtitle("Hours Per Iteration", subtitle = glue::glue("Dropping {nrow(x$iterations[hours_spent>=6])} Outliers"))
    ...
    ip = x$papers[,.(iters_paper = median(iterations_paper, na.rm= TRUE)), by = arrival_date]
    ip[, iterations_paper := data.table::frollmean(iters_paper,n = 11,na.rm = TRUE)]


    l$hours_p = ggplot(x$papers[ hours_spent < 10], aes(x = arrival_date, y = hours_spent)) + geom_point(size = 0.5) +
        geom_line(data = pp, aes(x = arrival_date, y = hours_spent), color = "red",size = 1) + theme_bw() +
        scale_x_date("Arrival Date",limits = c(as.Date("2020-01-01"), NA), breaks = seq(as.Date("2020-01-01"), as.Date("2023-10-01"), by = "6 months") ,date_labels = "%Y-%m") +
        geom_vline(xintercept = as.Date("2023-07-01")) + ggtitle("Hours Per Paper", subtitle = glue::glue("Dropping {nrow(x$papers[hours_spent >= 10])} Outliers"))

    l$iters_p = ggplot(x$papers, aes(x = arrival_date, y = iterations_paper)) + geom_point(size = 0.5) +
        geom_line(data = ip, aes(x = arrival_date, y = iterations_paper)  , color = "red",size = 1) + theme_bw() +
        scale_x_date("Arrival Date",limits = c(as.Date("2020-01-01"), NA),breaks = seq(as.Date("2020-01-01"), as.Date("2023-10-01"), by = "6 months") ,date_labels = "%Y-%m") +
        ggtitle("Iterations Per Paper")
...
plot_proctime <- function(x, by = "Iteration"){
    ...
    # base plot
    p = ggplot(mr, aes(x = arrival_date,y= Days, color = variable)) + geom_point(size = 0.5) +
        geom_line(data = haz, aes(x = arrival_date, y = smooth, color = variable),size = 1)
    ...
    days = p + ggplot2::theme_bw() + ggplot2::theme(legend.position = "bottom") +
        ...
        geom_hline(yintercept = hline_v, linetype = "dashed") +
...
#' Plot number and share of software used
#'
plot_software <- function(y){
    y[, completed := status %in% c("AP","NT", "P", "p"), by = ms]
    x = y[(completed), .(date_completed, software)]
    ...
    x[, lapply(.SD, mean,na.rm = TRUE), by = completed_quarter]
    d = x[, lapply(.SD, sum,na.rm = TRUE), .SDcols = stata:ztree]
    dd = data.table(Papers = t(d), Software = names(d))
    data.table::setnames(dd, "Papers.V1", "Papers")
    dd = rbind(dd, data.table(Papers = dd[Papers < 6, sum(Papers)], Software = "Other"))
    dd = dd[Papers >=6]
    dd = dd[order(Papers, decreasing = TRUE)]
    dd[, Share := Papers / sum(Papers)]
    dd = rbind(dd[-which(Software == "Other")], dd[which(Software == "Other")])

    data.table::setcolorder(dd, neworder = c("Software","Papers","Share"))
    dd
}
...
plot_DS <- function(x){
    ...
    m = melt(pd, id.vars = "arrival_quarter", value.name = "Share")
    ...
    ggplot(m, aes(x = arrival_quarter, y = Share)) + geom_line() +
        ggplot2::facet_wrap(~variable,ncol = 1, labeller = ggplot2::as_labeller(facet_names)) + theme_bw()
}
...
plot_hazard <- function(x){
    ...
    ggplot(x, aes(x = arrival_date, y = hazard_smooth, color = round)) + geom_line() +
        ggplot2::theme_bw() + ggplot2::theme(legend.position = "bottom") +
        geom_vline(xintercept = as.Date("2023-07-01"),size = 1) +
        geom_line(aes(y = ave(hazard_smooth, round, FUN = function(x){mean(x,na.rm = TRUE)})),size = 1) +
        ggtitle("Rejection Hazard by Round of Iteration")
